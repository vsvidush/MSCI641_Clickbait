...
output_file_path = 'predictions_XLNet.csv'  # File to save predictions
performance_file_path = 'performance_metrics_XLNet.csv'  # File to save performance metrics
checkpoint_dir = r'C:\Users\chawl\Documents\ClickBait\Model_XLNet_Task1\checkpoints'  # Directory to save checkpoints
...
start_epoch = load_checkpoint(xlnet_model, optimizer, scheduler, checkpoint_dir)
...
for epoch in range(start_epoch, EPOCHS):
    ...
    # Save checkpoint
    save_checkpoint(xlnet_model, optimizer, scheduler, epoch + start_epoch, checkpoint_dir)

# Generate predictions for the validation set
def get_predictions(model, data_loader, device, has_labels=True):
    model = model.eval()
    texts = []
    predictions = []
    prediction_probs = []
    ...
    with torch.no_grad():
        for d in data_loader:
            ...
            texts.extend(d['input_ids'].cpu().numpy())
            predictions.extend(preds)
            prediction_probs.extend(outputs.logits.cpu().numpy())
            if has_labels:
                labels = d["label"].to(device)
...
val_texts, val_preds, val_probs, val_labels = get_predictions(xlnet_model, val_loader, device)
...
# Calculate metrics
report = classification_report(val_labels, val_preds, target_names=label_encoder.classes_, output_dict=True)
...
test_texts, test_preds, test_probs = get_predictions(xlnet_model, test_loader, device, has_labels=False)
...
results_df = pd.DataFrame({'id': [f'{i}' for i in range(len(predicted_labels))], 'spoilerType': predicted_labels})
